...
## 三、代码内容  
- [PlayerMovement.cs](code/PlayerMovement.cs)：角色移动脚本（Top-Down用Rigidbody2D）。

- [ExpSystem.cs](code/ExpSystem.cs)：经验管理系统（经验球拾取、三选一升级界面）。  

## 四、图片内容  
② 核心玩法详解
1. 用户旅程图（核心循环）
核心体验不是“刷到强就赢”，而是每一步都要为下一步的风险收益付出“秩序值/混沌值”的代价。
- ![核心循环流程图](images/core_loop_flowchart.png)：核心玩法循环（进入房间→清怪→升级→胜利→秩序值→选下一房间）。  
## 五、扩展说明  
